...
一般ニュース(NHKニュース、BBC News)など、普段自分から追わないジャンルも
含めて幅広く収集し、Anthropic API (`claude-sonnet-5`) で日本語3〜4行に要約した上で、
LINE Messaging APIのpush messageで毎朝8:00 (JST) に配信します。
...
- **技術系以外: 7件**(英語ソースは最大3件、残り4件以上は日本語ソース)
  ...
  - ソース: NHKニュース(主要/国際/経済), BBC News(World/Business)

要約は以下の方針でAnthropic APIに指示しています(`scripts/send_news.py` の
...
ニュースソース自体を変更・追加したい場合は、同ファイルの `fetch_hackernews` /
...
呼び出すだけで組み込めます。`category` には `tech` / `world` / `finance` / `general`
を指定でき、`world` を指定した記事にのみ「フラットな要約」の追加指示が
プロンプトに付与されます。
