...
LINE Messaging APIのpush messageで毎朝5:43 (JST) 頃に配信します。
...
- `.github/workflows/daily-news.yml` が毎朝5:43 (JST) 頃に自動実行されます
  (cronはUTC基準のため `43 20 * * *` = UTC 20:43 = JST翌5:43 と設定しています)。
  Gemini無料枠のレート制限を避けるための待機を挟むため実行に数分かかること、
  毎時00分ちょうどはGitHub Actions全体で混雑し実行が遅延しやすいことに加え、
  実際の観測でスケジュール実行が毎回ほぼ一定(約1時間50分)遅れて発火して
  いたため、その分を見込んで大きく前倒ししています。GitHub Actionsのスケジュール
  実行はあくまでベストエフォートで正確な時刻を保証するものではなく、この
  遅延幅も将来変わる可能性がある点にご注意ください
